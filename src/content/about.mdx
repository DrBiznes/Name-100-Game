...
*"Name a single historical woman! Wait, not her - name another one! Keep going..."* <Note number={1}>This challenge originated from Twitch streamer QTCinderella in early 2024, quickly becoming a viral sensation in the streaming community.</Note>
...
But attempting the challenge and building a system to verify it are two very different beasts. When I started developing Name100Women, the skeptics had plenty to say:

- "The Wikipedia API will rate limit you to death" <Note number={3}>We solved this through aggressive caching and a multi-layered verification system that minimizes API calls.</Note>
...
### 🔍 Verification Vanessa: The Name Validation Saga

Our verification system evolved through three increasingly sophisticated layers: <Note number={4}>Each layer of verification adds both accuracy and complexity, requiring careful balance between speed and correctness.</Note>
...
#### 🎭 Mononym Mary: The Single-Name Sensation

The mononym issue first became apparent when users started entering names like "Cher" and "Beyoncé". <Note number={5}>Mononyms presented a unique challenge, as they break the traditional FirstName LastName pattern but are still valid entries.</Note> Our initial system rejected these as invalid since they didn't follow the expected "FirstName LastName" pattern. This led to the creation of our mononym handling system:
...
Found a missing name? Our databases are community-maintained and we welcome contributions! <Note number={6}>Community contributions have been vital in expanding our database coverage, especially for non-Western names and historical figures.</Note>

```json
// Example mononym addition to mononyms.json
{
  "name": "NewArtist",
...
#### 📚 Wiki Winona: The Wikipedia Warriors

When our local database check fails, we fall back to the Wikipedia API. This requires careful verification of several factors. <Note number={7}>The Wikipedia API provides rich data but requires careful parsing to determine if an article is about a woman.</Note>
...
We're always looking to improve Name100Women. Some features on our roadmap: <Note number={8}>These features are prioritized based on user feedback and technical feasibility.</Note>
