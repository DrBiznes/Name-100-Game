...
In early 2024, a simple challenge emerged that would expose an unsurprising gap in humanities collective knowledge: Name 100 women. NorthernLions attempt at the challenge inspired me to make this game!! To prove that any human can do it!! NAME 100 WOMEN!!<Note number={1}>This challenge originated from Twitch streamer QTCinderella in early 2024, quickly becoming a viral sensation in the streaming community.</Note>
...
APE FACE BIG PROBLEMS WHEN MAKE GAME!! APE NEED:

APE need check names FAST and GOOD!! not easy because names come from ALL OVER WORLD and ALL TIME PERIODS!! sometimes wikipedia say NO MORE API CALLS so APE need be SMART with cache!! 

some humans have WEIRD names like "Madonna" or "Cher" but APE handle this good!! APE also need figure out if human is woman when wikipedia page not clear!!
...
Validating various versions of voluminous names needed novel solutions - we're talking millions of magnificent monikers! The solution? A multi-layered verification system that gets progressively more precise (and computationally powerful) as needed.
...
This normalization is crucial - it handles everything from diacritics to hyphenated names. <Note number={3}>Name normalization is the unsung hero of the validation system, handling countless edge cases before they become problems.</Note>
...
1. APE CHECK LOCAL DATABASE FIRST (SUPER FAST!!)
...
The real magic happens in the gender verification logic: <Note number={7}>The gender verification system uses a combination of linguistic markers and context clues to determine if an article is about a woman.</Note>
...
1. go to **[mononyms.json](https://github.com/yourusername/name100women/edit/main/src/lib/mononyms.json)** on github
2. click edit button (looks like tiny pencil)
3. add new amazing mononym like this:
...
4. scroll down, write message like "ADD QUEEN BEY TO LIST!!" 
5. click "Propose changes" 
6. click "Create pull request"

THAT ALL!! APE CHECK WIKIPEDIA LINK AND MAKE SURE EVERYTHING GOOD!! <Note number={7}>APE NEED CHECK EACH NAME MANUALLY TO MAKE SURE IT WORK WITH VALIDATION SYSTEM!! APE SORRY FOR SLOW BUT APE WANT MAKE SURE EVERYTHING PERFECT!! 😢</Note>
...
This awesome game runs on GitHub Pages and Cloudflare's **SUPER STRONG WORKERS** and SQlite db!!!!!!!!! <Note number={8}>A "serverless" architecture means I don't maintain traditional servers - instead, the code runs automatically on Cloudflare's global infrastructure, as close to each player as possible.</Note>
...
me put game on cloudflare because cloudflare STRONG like APE!! game run FAST because APE know edge computing good!! <Note number={9}>APE BACKEND PROCESS 50,000 NAME CHECKS EVERY DAY AND RESPONSE TIME FASTER THAN BANANA PEEL!! APE SMART WITH 🦍 DISTRIBUTED SYSTEM AND CACHE STRATEGY!!</Note>
...
**Remember: IF NAME NOT WORK BUT SHOULD WORK, TELL APE!!** 🦍 🍌 **APE FIX!!** 🦍 🍌 **APE WANT MAKE SURE ALL AMAZING WOMEN CAN BE FOUND!!** 🦍 🍌 🦍
